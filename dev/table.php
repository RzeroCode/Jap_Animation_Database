	...
	if ($_GET["t"] === "1") {
		...
		$tablename = "Studios";
		$check = mysqli_query($link, "SELECT * FROM studio");
	} else if ($_GET["t"] === "3") {
		$tablename = "Employees";
		$check = mysqli_query($link, "SELECT * FROM employees,studio WHERE studio.StudioID = employees.StudioID");
	} else {
	                ...
	                <?php
						if ($_GET['t'] === "1") {
							...
							for ($i = 0; $i < count($myarr); $i++) {
								echo "<tr>";
								echo "<td>" . $myarr[$i]['StudioID'] . "</td>";
								echo "<td>" . $myarr[$i]['AnimationID'] . "</td>";
								...
								if (!empty($_SESSION['admin']))
									echo '<td  scope="row"><a class="btn btn-sm btn-success" id="Edit' . $myarr[$i]['AnimationID'] . '" href="singlerow.php?i=' . $myarr[$i]['AnimationID'] . '&t=1" role=" button">View</a><a class="btn  btn-sm btn-danger" href="actions/removeAnimationRow.php?i=' . $myarr[$i]['AnimationID'] . '&t=1" role="button">Remove</a></td>';
						...
						if ($_GET['t'] === "2") {
							...
							for ($i = 0; $i < count($myarr); $i++) {
								...
								echo "<td>" . $myarr[$i]['rating'] . "</td>";
								if (!empty($_SESSION['admin']))
									echo '<td  scope="row"><a class="btn btn-sm btn-success" id="Edit' . $myarr[$i]['StudioID'] . '" href="singlerow.php?i=' . $myarr[$i]['StudioID'] . '&t=2" role=" button">View</a><a class="btn  btn-sm btn-danger" href="actions/removeAnimationRow.php?i=' . $myarr[$i]['StudioID'] . '&t=2" role="button">Remove</a></td>';
						...
						if ($_GET['t'] === "3") {
							...
							if (!empty($_SESSION['admin']))
								echo '<th scope="col"></th>';
							echo '</tr></thead><tbody id="pageTable">';
							for ($i = 0; $i < count($myarr); $i++) {
								echo "<tr>";
								echo "<td>" . $myarr[$i]['EmployeeName'] . "</td>";
								echo "<td>" . $myarr[$i]['StudioName'] . "</td>";
								echo "<td>" . $myarr[$i]['Pay'] . "</td>";
								echo "<td>" . $myarr[$i]['Job'] . "</td>";
								if (!empty($_SESSION['admin']))
								echo '<td  scope="row"><a class="btn btn-sm btn-success" id="Edit' . $myarr[$i]['EmployeeID'] . '" href="singlerow.php?i=' . $myarr[$i]['EmployeeID'] . '&t=3" role=" button">View</a><a class="btn  btn-sm btn-danger" href="actions/removeAnimationRow.php?i=' . $myarr[$i]['EmployeeID'] . '&t=3" role="button">Remove</a></td>';
